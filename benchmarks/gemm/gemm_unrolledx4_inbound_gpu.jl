...
function gemm_kernel(N, M, K, A, B, C)
    ...
    if i <= N && j <= M
        ...
        @inbound for k = 1:4:K
            @inbound acc3 += alpha * A[i, k] * B[k, j]
            @inbound acc4 += alpha * A[i, k+1] * B[k+1, j]
            @inbound acc5 += alpha * A[i, k+2] * B[k+2, j]
            @inbound acc6 += alpha * A[i, k+3] * B[k+3, j]
        end

        # Handle the remaining values
        @inbound for k = (K - rem(K, 4)) + 1:K
            @inbound acc2 += alpha * A[i, k] * B[k, j]
        end

        @inbound C[i, j] = acc1 + acc2
    end
end
