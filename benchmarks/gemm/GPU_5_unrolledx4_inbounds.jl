...
# eliminate global variables + accumulators + unroll x4 + inbounds
...
function gemm_kernel(N, M, K, A, B, C)
    ...
    if i <= N && j <= M
        ...
        @inbounds for k = (K - rem(K, 4)) + 1:K
            @inbounds acc2 += alpha * A[i, k] * B[k, j]
        end

        @inbounds C[i, j] = acc1 + acc2 + acc3 + acc4 + acc5 + acc6
    end
    nothing 
end
