...
function doitgen_kernel(nr, nq, np, A, C4, sum)
    r, q, p, s = threadIdx().x, threadIdx().y, 1:np, 1:np
    offset_r = (blockIdx().x - 1) * blockDim().x
    offset_q = (blockIdx().y - 1) * blockDim().y

    if offset_r + r <= nr && offset_q + q <= nq
        for pp in p
            sum[pp] = 0.0
            for ss in s
                sum[offset_r + r, offset_q + q, pp] += A[offset_r + r, offset_q + q, ss] * C4[ss, pp]
            end
        end

        for pp in p
            A[offset_r + r, offset_q + q, pp] = sum[offset_r + r, offset_q + q, pp]
        end
    end
