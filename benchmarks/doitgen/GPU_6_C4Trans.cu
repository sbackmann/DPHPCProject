...
__global__ void kernel_doitgen(int nr, int nq, int np,
    ...
    if (r < nr && q < nq) {
        for(int p = 0; p < np; p++) {
            double tempSum = 0.0;
            for (int s = 0; s < np; s++) {
                tempSum += A[r * nq * np + q * np + s] * C4[p * np + s];
            }
...
__global__ void inplace_transpose(double* C4, int np) {
...
void run_doitgen_gpu(int nr, int nq, int np,
    ...
    dim3 numBlocks2(np / 16 + 1, np / 16 + 1);
    inplace_transpose<<<numBlocks2,threadsPerBlock>>>(C4, np);
