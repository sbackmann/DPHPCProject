...
function main()
    ...
    res = @dphpc_time((A, C4, sum) = init_array(nr, nq, np), doitgen(nr, nq, np, A, C4, sum), "S")
    ...
    np = 256
    A, C4, sum = init_array(nr, nq, np)
    res = @dphpc_time((A, C4, sum) = init_array(nr, nq, np), doitgen(nr, nq, np, A, C4, sum), "M")
    ...
    if ASSERT
        doitgen(nr, nq, np, A, C4, sum)
        assert_correctness(A, "L")
    end
    res = @dphpc_time((A, C4, sum) = init_array(nr, nq, np), doitgen(nr, nq, np, A, C4, sum), "L")
    ...
    np = 270
    A, C4, sum = init_array(nr, nq, np)
    res = @dphpc_time((A, C4, sum) = init_array(nr, nq, np), doitgen(nr, nq, np, A, C4, sum), "paper")
