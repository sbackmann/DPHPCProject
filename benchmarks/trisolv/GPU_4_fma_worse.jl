...
function pre_comp_kernel(L, Lx, x, j, N, inv)
    start_row = j

    i = (blockIdx().x - 1) * blockDim().x + (threadIdx().x - 1 ) + start_row
    if i <= N
        # if i == j
        #     Lx[i, j] = inv[i] * L[i, j] * x[j] + Lx[i, j - 1]
        # else
...
function kernel(L, x, b)
    ...
    neg_b_prod_inv_diag = CUDA.zeros(eltype(L), N)
    neg_b_prod_inv_diag = b .* inv_diag

    Lx = CUDA.zeros(eltype(L), N, N)

    x = CuArray(neg_b_prod_inv_diag)
    x *= -1
    CUDA.@allowscalar Lx[:, 1] = L[:, 1] * -neg_b_prod_inv_diag[1]
    
    for i in 2:N
        CUDA.@allowscalar x[i] += Lx[i, i-1] * inv_diag[i]
        
        blocks = ceil(Int, (N - i + 1) / t)
        @cuda threads=t blocks=blocks pre_comp_kernel(L, Lx, x, i, N, inv_diag)
    end
