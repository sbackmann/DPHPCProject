...
function main()
    tsteps, n = 50, 150
    A, B = init_arrays(n)
    res = @dphpc_time(init_arrays(n), kernel_j2d(tsteps, n, A, B), "S")
    println(res)

    tsteps, n = 80, 350
    A, B = init_arrays(n)
    res = @dphpc_time(init_arrays(n), kernel_j2d(tsteps, n, A, B), "M")
    println(res)

    tsteps, n = 200, 700
    A, B = init_arrays(n)
    res = @dphpc_time(init_arrays(n), kernel_j2d(tsteps, n, A, B), "L")
    println(res)

    tsteps, n = 1000, 2800
    A, B = init_arrays(n)
    res = @dphpc_time(init_arrays(n), kernel_j2d(tsteps, n, A, B), "paper")
    println(res)
end
