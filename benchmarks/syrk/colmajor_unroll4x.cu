...
__global__ void syrk(int n, int k, double alpha, double beta, double *C, double *A) {
    ...
    if (r < n && c < n && r >= c) {
        ...
        int i = 0;
        for (; i < k-3; i += 4) {
            s += get(A, n, i, r)   * get(A, n, i, c); // swap indeces
            s += get(A, n, i+1, r) * get(A, n, i+1, c); // swap indeces
            s += get(A, n, i+2, r) * get(A, n, i+2, c); // swap indeces
            s += get(A, n, i+3, r) * get(A, n, i+3, c); // swap indeces
        }
        for (; i < k; i++) {
            s += get(A, n, i, r) * get(A, n, i, c); // swap indeces
        }
    ...
    double* C, double* A)
{
    ...
    *beta = 1.2;
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < m; j++) {
            get(A, n, j, i) = (double) ((i*j+1)%n) / n; // swap indeces
        }
    }
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < n; j++) {
